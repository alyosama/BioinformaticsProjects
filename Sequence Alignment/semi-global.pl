...
for(my $i=0;$i<scalar @filenames;++$i){
  ...
  my $maxScore=semi_global($sequence_pairs[$i][0],$sequence_pairs[$i][1]);
  print "Alignment Score is $maxScore\n\n";

}

################################################################################
####################### Semi-Global Alignment Fuction ##########################
################################################################################
sub semi_global{
  # Match Score: m=5
  # Mismatch Score: s=-4
  ...
  for(my $i=0; $i <= (scalar @A); $i++) {
      ...
      if($i==0 or $j==0){
        ...
        if($A[$i-1] eq $B[$j-1]){
          $array->[$i][$j]=max($array->[$i-1][$j-1]+$m,$array->[$i][$j-1]+$d,$array->[$i-1][$j]+$d);
        }
        else{
          $array->[$i][$j]=max($array->[$i-1][$j-1]+$s,$array->[$i][$j-1]+$d,$array->[$i-1][$j]+$d);
        }
...
################ Backtracking and deducing the alignment.#####################
  ...
  do{
    if((($array->[$a][$b] == $array->[$a][$b-1]+$d ) and ($b!=0))or $a==0){
      unshift @sequence_A , '-';
      unshift @sequence_B , $B[$b-1];
      unshift @matching_str , ' ';
      $b--;
    }
    elsif((($array->[$a][$b] == $array->[$a-1][$b]+$d) and ($a!=0))or $b==0){
      unshift @sequence_A , $A[$a-1];
      unshift @sequence_B , '-';
      unshift @matching_str , ' ';
      $a--;
